...
int main(int argc, char *argv[]) {
  if (argc == 2) {
          ...
          if (existeRuta(disco, strArray, largo, inodo)) {
            if (largo > 1) {
              ...
              if (existeDirectorio(disco, strArray[largo - 1], posBloques)) {
                borrarDirectorio(disco, strArray[largo - 1], posBloques);
                if (shmdt(disco) == -1) {
              ...
              cout << "Error '" << argv[1] << "' no es un directorio" << endl;
            }
          } else {
    ...
    cout << "Error ingrese correctamente el directorio que desea borrar" << endl;
  }
}
